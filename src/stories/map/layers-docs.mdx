...
## Using native OpenLayers layers

In addition to the built-in composable layers, you can use native OpenLayers layers via the NativeLayerWrapper.
...
### Mixing with custom layers

Native layers can be used alongside composable layers:

```ts
emMap.addLayer(new LocationsLayer({ positions }))
...
All wrapped native layers support the same interaction API:

```ts
const layer = new NativeLayerWrapper(new VectorLayer(...))
...
If your data includes a direction (bearing), you can use it.cCan specify degrees or radians:
...
Entry - Uses **first point direction**, reversed
Exit - Uses **last point direction**, as-is
