...
Overlays rows are capable of making use of copy link functionality which allows the user to copy a value to their clipboard via a copy anchor element.

To enable this on a row you need to add an anchor element with the class `app-map__copy-link` and another element with the attribute `data-copy-value`
